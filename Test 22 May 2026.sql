...
select * from worker
where salary between 100000 and 500000 or Department = 'admin';
...
select department, count(worker_id) as number_of_people from worker
...
select department, count(worker_id) as total_people from worker
group by department;
